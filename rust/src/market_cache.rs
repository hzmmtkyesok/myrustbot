...
const ATP_BUFFER: f64 = TENNIS_BUFFER;
const LIGUE1_BUFFER: f64 = SOCCER_BUFFER;
...
// Aliases for backwards compatibility
impl CacheStats {
    pub fn atp_count(&self) -> &AtomicU64 { &self.tennis_count }
    pub fn ligue1_count(&self) -> &AtomicU64 { &self.soccer_count }
}
...
impl MarketCaches {
    ...
    pub fn is_ligue1_token(&self, token_id: &str) -> bool {
        self.is_soccer_token(token_id)
    }

    /// Get ATP/tennis buffer for token (0.01 if tennis, 0.0 otherwise)
    #[inline]
    ...
    pub fn get_atp_buffer(&self, token_id: &str) -> f64 {
        self.get_tennis_buffer(token_id)
    }

    /// Get Ligue 1/soccer buffer for token (0.01 if soccer, 0.0 otherwise)
    #[inline]
    ...
    pub fn get_ligue1_buffer(&self, token_id: &str) -> f64 {
        self.get_soccer_buffer(token_id)
    }
...
/// Get ATP buffer for a token (convenience function) - alias
#[inline]
...
/// Get Ligue 1 buffer for a token (convenience function) - alias
#[inline]
